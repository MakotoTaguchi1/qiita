...
tags:
  ...
  - 生成AI
private: false
updated_at: '2026-01-03T13:36:58+09:00'
...
現在の ChatGPT・Claude・Gemini などは、細部の設計は違っても**土台は Transformer 系**です。特に GPT 系は「Transformer のデコーダだけを積んだ構造」になっています。
...
この仕組みを理解すると、**LLM が得意なこと／苦手なこと**が自然に見えてきます。  
次の節では、Seq2Seq と Transformer の違いを整理していきます。
...
エンコーダ・デコーダいずれも入力時は、以下 2 つの処理が走ります。
...
※ ここでは機械翻訳のような **エンコーダ＋デコーダ型** を例に説明します。  
GPT 系はデコーダのみのため、後半の「エンコーダを参照する処理」は登場しません。

### エンコーダ：入力文を「理解する」

まずエンコーダは、入力された文章全体を受け取ります。

エンコーダ内部では、各層で次のような処理が行われます。

- Self-Attention により、単語同士の関係性を捉える
- 文脈を踏まえた情報を、ニューラルネットワークで整理・変換する
...
これらをもとに、**次に来る単語を 1 つ決定**します。

#### ① Masked Self-Attention：ここまでに書いた文脈を見る

まずデコーダは、**これまでに自分が生成した単語列**だけを対象に Self-Attention を行います。

このとき、未来の単語は見えないように **マスク** されています。

これにより、「先の答えを知った状態で文章を書く」ことを防ぎ、**人間が文章を書くのと同じ条件**で処理が行われます。

#### ② エンコーダの出力と照らし合わせる（入力文を参照する）

次にデコーダは、**エンコーダから渡された文脈情報を参照**します。
ここで行われるのは、今生成しようとしている単語が**入力文のどの部分と関係が深いか**を判断する処理です。
...
このように、入力文（Source）と生成中の文（Target）の間で行われる Attention は、
一般に「**Source-Target Attention**（または Cross-Attention）」と呼ばれます。

#### ③ Feed Forward：次の単語を決める準備

最後に、ニューラルネットワーク（Feed Forward）によって、
